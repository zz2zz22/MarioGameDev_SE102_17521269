...
bool Pipeline::CreateInputLayout(D3D10_INPUT_ELEMENT_DESC* layout, unsigned int elements) {
	...
	HRESULT hResult = GameUtils::directDevice->CreateInputLayout(
		layout,
		elements,
	...
	GameUtils::directDevice->IASetInputLayout(_inputLayout);
...
bool Pipeline::CreateRenderTagetView() {
	...
	hResult = GameUtils::directDevice->CreateRenderTargetView(backBuffer, nullptr, &_renderTargetView);
	backBuffer->Release();
	if (FAILED(hResult)) {
	...
	GameUtils::directDevice->OMSetRenderTargets(1, &_renderTargetView, nullptr);
...
bool Pipeline::CreateViewport(unsigned int backBufferWidth, unsigned int backBufferHeight) {
	...
	viewPort.TopLeftX = 0;
	viewPort.TopLeftY = 0;
	GameUtils::directDevice->RSSetViewports(1, &viewPort);

	//Creates the sprite handler;
	HRESULT hResult = D3DX10CreateSprite(GameUtils::directDevice, 0, &GameUtils::spriteHandler);
	if (FAILED(hResult)) {
	...
	hResult = GameUtils::spriteHandler->SetProjectionTransform(&projectionMatrix);
	if (FAILED(hResult)) {
...
bool Pipeline::CreateRasterizerState() {
	...
	HRESULT hResult = GameUtils::directDevice->CreateRasterizerState(&rasterizerDesc, &_rasterizerState);
	if (FAILED(hResult)) {
		OutputDebugStringA("[PIPELINE] Failed to create rasterizer state\n");
		return false;
	}
	GameUtils::directDevice->RSSetState(_rasterizerState);
...
bool Pipeline::CreateBlendState() {
	...
	HRESULT hResult = GameUtils::directDevice->CreateBlendState(&blendDesc, &_blendState);
	if (FAILED(hResult)) {
...
void Pipeline::Release() {
	if (GameUtils::font != nullptr) {
		GameUtils::font->Release();
	}
	...
	if (GameUtils::spriteHandler != nullptr) {
		GameUtils::spriteHandler->Release();
	}
	...
	if (GameUtils::directDevice != nullptr) {
		GameUtils::directDevice->Release();
	}
