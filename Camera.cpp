...
RECTF Camera::GetViewport() const {
	...
	RECTF viewport;
	viewport.left = _position.x - GameUtils::SCREEN_WIDTH * REGION_OFFSET.left;
	viewport.top = _position.y - GameUtils::SCREEN_HEIGHT * REGION_OFFSET.top;
	viewport.right = _position.x + GameUtils::SCREEN_WIDTH * REGION_OFFSET.right;
	viewport.bottom = _position.y + GameUtils::SCREEN_HEIGHT * REGION_OFFSET.bottom;
	return viewport;
}
